...
└── src/
    ...
    └── views/                  # 페이지 뷰 (도메인별 폴더 그룹)
        ...
        │   ├── MemberChatView.vue          # [미구현] 채팅 (목 데이터)
        │   ├── MemberManualView.vue        # [미구현] 운동 매뉴얼 목록 (목 데이터)
        │   └── ManualDetailView.vue        # [미구현] 매뉴얼 상세 (목 데이터)
        │
        └── trainer/
            ...
            ├── MemoWriteView.vue               # [미구현] 메모 작성 (목 데이터)
            ├── TrainerChatView.vue             # [미구현] 채팅 (목 데이터)
            ├── TrainerManualView.vue           # [미구현] 매뉴얼 관리 (목 데이터)
            ├── ManualRegisterView.vue          # [미구현] 매뉴얼 등록 (목 데이터)
            ├── TodayWorkoutView.vue            # [미구현] 오늘의 운동 (목 데이터)
            ├── MemberPaymentView.vue           # [미구현] 수납 기록 (목 데이터)
            └── PaymentWriteView.vue            # [미구현] 수납 작성 (목 데이터)
```
...
| `trainer_members` | 트레이너-회원 연결 관계 (초대 코드 기반) |
...
| `create_reservation(...)` | 예약 생성. 트레이너-회원 연결 확인 + 시간 충돌 방지 |
...
| 기능 | 관련 뷰 | 현재 상태 |
|------|---------|----------|
| 채팅 | TrainerChatView, MemberChatView | 목 데이터 |
| 운동 매뉴얼 | TrainerManualView, ManualRegisterView, MemberManualView, ManualDetailView | 목 데이터 |
| 오늘의 운동 | TodayWorkoutView | 목 데이터 |
| 수납 기록 | MemberPaymentView, PaymentWriteView | 목 데이터 |
| 메모 작성 | MemoWriteView | 목 데이터 |
| 알림 시스템 | — | 미구현 |
| 계정 삭제 | — | 미구현 |
| 프로필 수정 | — | 미구현 (온보딩 시 생성만) |
